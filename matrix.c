...
{
	left.x = fmin(fmax(left.x, 0), 1);
	left.y = fmin(fmax(left.y, 0), 1);
	left.z = fmin(fmax(left.z, 0), 1);
	left.w = fmin(fmax(left.w, 0), 1);
...
{
	dest->x = fmin(fmax(left->x, 0), 1);
	dest->y = fmin(fmax(left->y, 0), 1);
	dest->z = fmin(fmax(left->z, 0), 1);
	dest->w = fmin(fmax(left->w, 0), 1);
}
